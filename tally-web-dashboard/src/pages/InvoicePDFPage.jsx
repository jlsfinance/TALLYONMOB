...
export default function InvoicePDFPage() {
    ...
    // Load invoice template settings from localStorage
    const [invoiceSettings, setInvoiceSettings] = useState(() => {
        if (!selectedCompany?.id) return null;
        try {
            const raw = localStorage.getItem(`invoice_template_${selectedCompany.id}`);
            return raw ? JSON.parse(raw) : null;
        } catch { return null; }
    });

    const templateId = invoiceSettings?.template || 'tally-classic';
    const fieldToggles = invoiceSettings?.fields || {
        showLogo: true, showGST: true, showAddress: true, showPhone: true, showEmail: true,
        showHSN: true, showQR: false, showTerms: true, showBank: true, showSignature: true,
        showDiscount: true, showNarration: true
    };
    const templateColors = invoiceSettings?.colors || { primary: '#000000', accent: '#f5f5f5', headerBg: '#ffffff', headerBorder: '#000000' };
    const customLogoUrl = invoiceSettings?.logoUrl || '';
    const customTermsText = invoiceSettings?.termsText || '1. Goods once sold will not be taken back.\n2. Interest @18% p.a. will be charged if payment is not made within due date.\n3. Subject to local jurisdiction.';
    ...
    useEffect(() => {
        if (id && selectedCompany?.id) {
            ...
            try {
                const raw = localStorage.getItem(`invoice_template_${selectedCompany.id}`);
                if (raw) setInvoiceSettings(JSON.parse(raw));
            } catch {}
        }
    ...
    const generatePDF = async (action = 'download') => {
        ...
        const primaryRgb = hexToRgb(templateColors.primary);
        const headerBgRgb = hexToRgb(templateColors.headerBg);
        ...
        if (templateId !== 'tally-classic') {
            doc.setFillColor(headerBgRgb.r, headerBgRgb.g, headerBgRgb.b);
            doc.rect(margin, y, pageWidth - 2 * margin, headerH, 'F');
        }
        ...
        doc.setFont('helvetica', 'bold');
        // Use white text on colored header, black on classic
        if (templateId !== 'tally-classic') {
        ...
        doc.setFontSize(10);
        doc.setFont('helvetica', 'bold');
        // Use white text on colored header for non-classic templates
        if (templateId !== 'tally-classic') {
            doc.setTextColor(255, 255, 255);
        } else {
            doc.setTextColor(0, 0, 0);
        ...
        autoTable(doc, {
            ...
            headStyles: {
                fillColor: templateId !== 'tally-classic' ? [headerBgRgb.r, headerBgRgb.g, headerBgRgb.b] : [255, 255, 255],
                textColor: templateId !== 'tally-classic' ? [255, 255, 255] : [0, 0, 0],
                fontStyle: 'bold',
                halign: 'center',
                lineWidth: 0.1,
                lineColor: [0, 0, 0]
                        ...
                        <div className={`${isMobile ? 'p-2' : 'p-6 md:p-8'} h-full flex flex-col relative`}>
                            ...
                            <div className="border-2 border-black h-full flex flex-col text-[10px] md:text-xs">
                                ...
                                <div className="border-b-2 border-black">
                                    ...
                                    <div className="md:hidden">
                                        <div className="flex items-center justify-between p-1 border-b border-black"
                                            style={{ backgroundColor: templateColors.headerBg, color: templateId === 'tally-classic' ? '#000' : '#fff' }}>
                                        ...
                                        <div className="grid grid-cols-[1fr_auto] gap-x-2 p-1">
                                            <div className="min-w-0">
                                                ...
                                                {fieldToggles.showGST && companyInfo?.gstin && <p className="text-[7px]"><span className="font-semibold">GSTIN:</span> {companyInfo.gstin}</p>}
                                                {companyInfo?.state && <p className="text-[7px]">{companyInfo.state}</p>}
                                            </div>
                                            <div className="text-right text-[7px] space-y-px">
                                                {fieldToggles.showAddress && companyInfo?.address && <p className="truncate max-w-[120px]">{companyInfo.address}</p>}
                                    ...
                                    <div className="hidden md:grid md:grid-cols-2">
                                        ...
                                        <div className="p-4 border-r-2 border-black flex flex-col justify-center"
                                            style={templateId !== 'tally-classic' ? { backgroundColor: templateColors.accent + '33' } : {}}>
                                            <div className="flex items-start gap-3 mb-1">
                                                {fieldToggles.showLogo && customLogoUrl && (
                                                    <img src={customLogoUrl} alt="Logo" className="h-10 w-10 object-contain" crossOrigin="anonymous" />
                                                )}
                                                {fieldToggles.showLogo && !customLogoUrl && (
                                                    <div className="w-10 h-10 bg-gray-200 rounded flex items-center justify-center text-[8px] text-gray-400 shrink-0">LOGO</div>
                                                )}
                                                <h1 className="text-lg font-bold uppercase tracking-tight">{companyInfo?.name}</h1>
                                            </div>

                                            {fieldToggles.showAddress && (companyInfo?.address) && (
                                            ...
                                            <div className="text-xs space-y-0">
                                                {fieldToggles.showGST && (companyInfo?.gstin) && (
                                                ...
                                                {fieldToggles.showEmail && (companyInfo?.email) && (
                                                    <p><span className="font-semibold">Email:</span> {companyInfo.email}</p>
                                                )}
                                                {fieldToggles.showPhone && (companyInfo?.phone) && (
                                        ...
                                        <div className="flex flex-col">
                                            <div className="p-2 border-b-2 border-black text-center"
                                                style={{ backgroundColor: templateColors.headerBg, color: templateId === 'tally-classic' ? '#000' : '#fff' }}>
                                ...
                                <div className="border-b-2 border-black p-0">
                                    ...
                                    <div className="md:hidden flex items-center justify-between p-1 border-b border-black"
                                        style={{ backgroundColor: templateColors.accent + '55' }}>
                                    ...
                                    <div className="hidden md:block">
                                        <div className="px-2 py-1 text-[10px] font-bold border-b border-black uppercase"
                                            style={{ backgroundColor: templateColors.accent + '55' }}>Buyer (Bill to)</div>
                                ...
                                <div className="flex-grow flex flex-col border-b-2 border-black relative overflow-x-auto">
                                    {/* Table Header */}
                                    <div className={`flex text-[7px] md:text-[10px] font-bold border-b border-black text-center ${templateId !== 'tally-classic' ? '' : 'bg-gray-50'}`}
                                        style={templateId !== 'tally-classic' ? { backgroundColor: templateColors.accent + '55' } : { backgroundColor: '#f9fafb' }}>
                                        ...
                                        {columnVisibility.hasHSN && fieldToggles.showHSN && <div className="w-8 md:w-16 p-0.5 md:p-1.5 border-r border-black hidden sm:block">HSN</div>}
                                        ...
                                        {columnVisibility.hasDiscount && fieldToggles.showDiscount && <div className="w-8 md:w-16 p-0.5 md:p-1.5 border-r border-black hidden lg:block">Disc</div>}
                                    ...
                                    <div className={`flex-grow text-[8px] md:text-[10px] relative ${isMobile ? '' : 'min-w-[400px]'}`}>
                                        {items.map((item, idx) => {
                                            ...
                                            return (
                                                    ...
                                                    {columnVisibility.hasHSN && fieldToggles.showHSN && <div className="w-8 md:w-16 p-0.5 md:p-1.5 border-r border-black text-center hidden sm:block">{item.hsn_code || ''}</div>}
                                                    ...
                                                    {columnVisibility.hasDiscount && fieldToggles.showDiscount && <div className="w-8 md:w-16 p-0.5 md:p-1.5 border-r border-black text-right hidden lg:block">
                                    ...
                                    {/* Totals Row */}
                                    <div className="flex border-t-2 border-black font-bold text-[8px] md:text-[10px]" style={{ backgroundColor: templateColors.accent + '33' }}>
                                        ...
                                        {columnVisibility.hasHSN && fieldToggles.showHSN && <div className="w-8 md:w-16 p-0.5 md:p-1.5 border-r border-black hidden sm:block"></div>}
                                        ...
                                        {columnVisibility.hasDiscount && fieldToggles.showDiscount && <div className="w-8 md:w-16 p-0.5 md:p-1.5 border-r border-black hidden lg:block"></div>}
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 border-b-2 border-black text-[8px] md:text-[10px]">
                                    ...
                                    <div className="text-right">
                                        ...
                                        <div className="flex justify-between p-1 md:p-2 font-bold text-[9px] md:text-sm border-t border-black"
                                            style={{ backgroundColor: templateColors.accent + '55' }}>
                                            <span>Total (INR)</span>
                                ...
                                <div className="md:hidden min-h-[3rem]">
                                    <div className="grid grid-cols-2 gap-0 text-[7px]">
                                        ...
                                        <div className="p-1 border-r border-b border-black">
                                            {fieldToggles.showBank && (companyInfo?.bank_name || companyInfo?.bank_account) && (
                                            ...
                                            {fieldToggles.showTerms && <p className="text-[6px] opacity-70">{customTermsText.split('\n')[0] || 'We declare that this invoice shows the actual price of the goods described.'}</p>}
                                        </div>
                                        {/* Right: QR + Signatory */}
                                        <div className="p-1 flex flex-col items-center justify-between">
                                            {fieldToggles.showQR && upiId && (
                                                <img
                                    ...
                                    <div className="col-span-2 border-r-2 border-black p-2 text-[10px] flex flex-col justify-between h-full">
                                        ...
                                        {fieldToggles.showTerms && (
                                            <div className="mt-2 text-[10px]">
                                                ...
                                                <p>{customTermsText || 'We declare that this invoice shows the actual price of the goods described and that all particulars are true and correct.'}</p>
                                            </div>
                                        )}
                                    </div>
