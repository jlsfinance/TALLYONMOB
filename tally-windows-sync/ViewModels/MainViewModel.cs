...
namespace TallySyncApp.ViewModels
{
    public class MainViewModel : INotifyPropertyChanged
    {
        ...
        public MainViewModel()
        {
            ...
            // AUTO-VALIDATE LICENSE on startup (1 second delay for UI to load)
            _ = Task.Run(async () =>
            {
                await Task.Delay(1000);
                Application.Current?.Dispatcher.Invoke(async () => await ValidateLicense());
            });
        }
        ...
        private async Task ValidateLicense()
        {
            ...
            if (string.IsNullOrEmpty(password))
            {
                ...
                bool isSuperAdminSession = email?.Trim().ToLower() == "lovneetrathi@gmail.com";
                IsLicenseValid = true;
                LicenseStatus = isSuperAdminSession ? "✅ Active (Super Admin)" : "✅ Active (Session)";
                LicensePlan = isSuperAdminSession ? "⭐ Pro (Admin)" : "🆓 Trial";
                LicenseDaysRemaining = isSuperAdminSession ? 9999 : 7;
                Log($"✅ License validated via existing auth session.{(isSuperAdminSession ? " [SUPER ADMIN]" : "")}");
                return;
            }
            
            var result = await _licenseService.ValidateLicenseAsync(email, password, tallySerial);

            // SUPER ADMIN BYPASS: lovneetrathi@gmail.com always gets Pro + unlimited
            bool isSuperAdmin = email?.Trim().ToLower() == "lovneetrathi@gmail.com";

            if (result.IsValid || isSuperAdmin)
            {
                IsLicenseValid = true;
                LicenseStatus = isSuperAdmin ? "✅ Active (Super Admin)" : "✅ Active";
                LicensePlan = isSuperAdmin ? "⭐ Pro (Admin)" : (result.IsPro ? "⭐ Pro" : "🆓 Trial");
                LicenseDaysRemaining = isSuperAdmin ? 9999 : result.DaysRemaining;
                Log($"✅ License valid! Plan: {result.Plan}, Days remaining: {result.DaysRemaining}{(isSuperAdmin ? " [SUPER ADMIN]" : "")}");

                if (!isSuperAdmin && result.ShouldShowUpgradePrompt)
                {
