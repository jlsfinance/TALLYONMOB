...
namespace TallySyncApp.Services
{
    ...
    public class ApiClient : IDisposable
    {
        ...
        {
            // Use Supabase PostgREST endpoint
            ...
            try
            {
                ...
                return (false, $"Cloud error: {response.StatusCode}");
            }
            catch (Exception ex)
            {
        ...
        private async Task<ApiResponse<T>> UpsertViaRawSqlAsync<T>(string table, object normalizedPayload, string onConflict)
        {
            ...
            // Use Supabase PostgREST native upsert
            ...
            var json = JsonConvert.SerializeObject(rows, serializerSettings);
            ...
            if (!response.IsSuccessStatusCode)
            {
                var errorMsg = responseContent.Length > 300 ? responseContent.Substring(0, 300) : responseContent;
                SyncLogger.Log($"UPSERT FAILED [{table}] HTTP {(int)response.StatusCode}: {errorMsg}");
                return new ApiResponse<T>
                {
                    Success = false,
                    Error = $"Upsert failed: {response.StatusCode} - {errorMsg}"
                };
            }

            return new ApiResponse<T> { Success = true, Message = $"Synced {rows.Count} rows to {table}" };
        }
