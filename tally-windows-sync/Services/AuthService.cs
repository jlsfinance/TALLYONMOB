    ...
    public class AuthService
    {
        ...
        public async Task<bool> RefreshTokenAsync()
        {
            ...
            try
            {
                ...
                // Don't sign out on refresh failure - just return false
                // The caller can handle fallback (anon key, etc.)
                return false;
            }
            catch
            {
        ...
        public async Task<string?> RefreshTokenIfNeededAsync()
        {
            ...
            if (effectiveExpiry <= DateTime.UtcNow.AddMinutes(5))
            {
                ...
                // Don't sign out on refresh failure - let caller handle fallback
                return CurrentSession?.AccessToken;
            }
