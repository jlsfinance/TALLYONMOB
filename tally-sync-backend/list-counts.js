const { Client, Databases } = require('node-appwrite');
const fs = require('fs');

const appwriteClient = new Client()
    .setEndpoint('https://nyc.cloud.appwrite.io/v1')
    .setProject('69a06098003b91c827a9')
    .setKey('standard_');

const databases = new Databases(appwriteClient);

async function run() {
    try {
        let lines = [];
        const res = await databases.listCollections('tally_sync_db');
        for (let c of res.collections) {
            try {
                const docs = await databases.listDocuments('tally_sync_db', c.$id);
                lines.push(`Collection ID: ${c.$id} | Name: ${c.name} | Total Docs: ${docs.total}`);
            } catch (e) {
                lines.push(`Collection ID: ${c.$id} | Error: ${e.message}`);
            }
        }
        fs.writeFileSync('out_utf8.txt', lines.join('\n'), 'utf8');
        console.log("Done");
    } catch (e) {
        console.error(e.message);
    }
}
run();
